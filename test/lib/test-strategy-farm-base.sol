...
contract StrategyFarmTestBase is DSTestSushiBase { // !!! update all
    address want;
    address token1;
    ...
    function _test_withdraw_release() internal {
        ...
        IERC20(want).safeApprove(address(daiJar), 0);
        IERC20(want).safeApprove(address(daiJar), _want);
        daiJar.deposit(_want);
        daiJar.earn();
    ...
    function _test_get_earn_harvest_rewards() internal {
        ...
        uint256 _before = daiJar.balance();
